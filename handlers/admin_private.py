...
)  # Делаем проверку на состояние AddProduct..
async def add_name(message: types.Message, state: FSMContext):
    if message.text == ".":
        await state.update_data(name=AddProduct.product_for_change.name)
    else:
        # Тут можно сделать дополнительную проверку
        ...
        await state.update_data(
            name=message.text
        )  # Записываем полученные от пользователя данные ключ: name - значение: text
...
async def add_description(message: types.Message, state: FSMContext):
    ...
    await state.set_state(AddProduct.price)  # Ожидаем ввода цены
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer(
            f"Ошибка: \n{str(e)}\nОбратитесь к программисту, для решения проблемы",
